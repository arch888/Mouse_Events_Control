...
def make_grid(name, *src):
    dst = np.zeros((int(height/2*3) , int(width/2*4)), np.uint8)
    added = 0
    for var in src:
        x1 = int((added%4)*width/2)
        y1 = int(math.floor(added/4)*height/2)
        x2 = int(x1 + width/2)
        y2 = int(y1 + height/2)
        dst[y1:y2, x1:x2] = cv2.resize(var, (int(width/2) , int(height/2)))
        added += 1
    cv2.imshow(name, dst)
...
cap = cv2.VideoCapture(0) #otwarcie kamery nr 1
...
while True: #operacje wykonywane dla kazdej przechwyconej klatki
    ...
    contours, _ = cv2.findContours(eroded.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #znajdowanie krawedzi
    ...
    if len(shapes) > 0:
        ...
        contours2, _ = cv2.findContours(exp.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #znajdowanie krawedzi
        ...
        if (len(x_vert)>0 and len(y_vert)>0):
            ...
            if mouse_enabled:
                if delta[0] > 2.2:
                    mouse_pos = autopy.mouse.location()
            ...
            for i in range(defects.shape[0]):
                ...
                b = math.sqrt((far[0] - start[0]) ** 2 + (far[1] - start[1]) ** 2)
                c = math.sqrt((end[0] - far[0]) ** 2 + (end[1] - far[1]) ** 2)
                angle = math.acos(clean_cos((b ** 2 + c ** 2 - a ** 2) / (2 * b * c))) * 57
                if angle <= 90:
                    count_defects += 1
                    cv2.circle(img, far, 2, [255, 0, 0], -1)
